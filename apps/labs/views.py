...
class RecruitmentStatusViewSet(viewsets.ModelViewSet):
    ...
    def partial_update(self, request, *args, **kwargs):
        """Partial update recruitment status"""
        # Clear cache when updating
        lab_id = kwargs.get('lab_id')
        if lab_id:
            CacheManager.invalidate_lab_caches(lab_id)
    ...
    @swagger_auto_schema(
        operation_summary="모집 현황 목록 조회",
        operation_description="모든 연구실의 모집 현황을 조회합니다. 필터링 가능합니다.",
        manual_parameters=[
            openapi.Parameter(
                'recruiting_phd',
                openapi.IN_QUERY,
                description='PhD 모집 중인 연구실만 조회 (true/false)',
                type=openapi.TYPE_BOOLEAN,
            ),
            openapi.Parameter(
                'recruiting_postdoc',
                openapi.IN_QUERY,
                description='Postdoc 모집 중인 연구실만 조회 (true/false)',
                type=openapi.TYPE_BOOLEAN,
            ),
            openapi.Parameter(
                'recruiting_intern',
                openapi.IN_QUERY,
                description='Intern 모집 중인 연구실만 조회 (true/false)',
                type=openapi.TYPE_BOOLEAN,
            ),
        ],
        responses={
            200: openapi.Response('조회 성공', recruitment_status_response),
        }
    )
    def list(self, request, *args, **kwargs):
        """List all recruitment statuses with optional filtering"""
        return super().list(request, *args, **kwargs)

    @swagger_auto_schema(
        operation_summary="모집 현황 상세 조회",
        operation_description="특정 연구실의 모집 현황을 조회합니다.",
        responses={
            200: openapi.Response('조회 성공', recruitment_status_response),
            404: openapi.Response('연구실을 찾을 수 없음', error_response),
        }
    ...
    def retrieve(self, request, *args, **kwargs):
        """Retrieve recruitment status for a specific lab"""
        return super().retrieve(request, *args, **kwargs)
